...
func appGetNotification(w http.ResponseWriter, r *http.Request) {
	...
	fare, err := rideCalculateDiscountedFare(ctx, db, user.ID, ride, ride.PickupLatitude, ride.PickupLongitude, ride.DestinationLatitude, ride.DestinationLongitude)
	if err != nil {
		writeError(w, http.StatusInternalServerError, err)
		return
	}
	...
	if ride.ChairID.Valid {
		...
		stats, err := noTxGetChairStats(ctx, db, chair.ID)
		if err != nil {
			writeError(w, http.StatusInternalServerError, err)
			return
		}
